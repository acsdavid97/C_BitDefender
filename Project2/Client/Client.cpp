...
TCHAR *sFilePath = NULL;
TCHAR *sUserName = _T("david");
TCHAR *sPassword = _T("defaultpassword");
TCHAR *sPipeName = _T("defaultpipename");
TCHAR *sOutPutPath = NULL;
TCHAR *sKey = NULL;
...
void validateArguments()
{
	...
	if (sOutPutPath == NULL || _tcslen(sOutPutPath) <= 0) {
		sOutPutPath = (TCHAR*)malloc(sizeof(TCHAR) * (_tcslen(sFilePath) + 4));
		if (sOutPutPath == NULL) {
...
{
	TCHAR *sRealPipeName = getRealPipeName(sPipeName);
	DWORD dwMode;
	BOOL bSuccess;
	HANDLE hPipe;
...
BOOL sendPacket(HANDLE hPipe, TCHAR *buff, DWORD cbPacketLen)
{
	DWORD cbWritten;
	BOOL bSuccess;
...
BOOL getPacket(HANDLE hPipe, TCHAR *buff, DWORD cbPacketLen)
{
	DWORD cbRead;
	BOOL bSuccess;
...
int _tmain(int argc, TCHAR *argv[])
{
	...
	if (!initilizeConnection(hPipe)) {
		_tprintf(_T("Could not initialize connection (server if probably busy)\n"));
		CloseHandle(hFileSource);
