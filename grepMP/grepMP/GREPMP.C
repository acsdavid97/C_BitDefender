	...
	in the current directory, to receive the results. */
{
	...
	LPBOOL lpbProcTerminated;

	DWORD finishedProcesses;
#ifdef UNICODE
	...
	lpbProcTerminated = malloc((argc - 2) * sizeof(BOOL));
	...
	finishedProcesses = 0;
	_tprintf(_T("Waiting for processses\n"));
	while (finishedProcesses < argc	- 2) {
		for (iProc = 0; iProc < argc-2; iProc++) {
			if (WaitForSingleObject(hProc[iProc], 0) == WAIT_OBJECT_0) {
				_tprintf(_T("Process number %d finished\n"), iProc);
				finishedProcesses++;
			}
		}
	}

	_tprintf(_T("all processes finished\n"));
